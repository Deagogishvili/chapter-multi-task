...
from PROT.base import TrainerBase, AverageMeter
...
class Trainer(TrainerBase):
    ...
    def __init__(self, model, loss, metrics, metrics_task, optimizer, start_epoch, config, device,
                 data_loader, batch_transform, method=None, optimizer_loss=None, valid_data_loader=None, lr_scheduler=None, trial=None, not_passed=True, accum_iter = None):
        super().__init__(model, loss, metrics, metrics_task, optimizer, start_epoch, config, device, not_passed)
        ...
        self.method=method
        self.log_step = int(data_loader.batch_size * 100)
        self.batch_transform = batch_transform
        self.trial = trial
        self.accum_iter = accum_iter
        self.accum_iter = accum_iter
        print('GRADIENT ACCUMULATION',self.accum_iter)
    ...
    def _train_epoch(self, epoch: int) -> dict:
        ...
        for batch_idx, (data, target, mask) in enumerate(self.data_loader):
            ...
            if self.method:
                loss, self.not_passed = self.loss(output, target, self.method, self.not_passed)
            else:
                loss, self.not_passed = self.loss(output, target)
            loss.backward()

            # we implement gradient accumulation in order
            # to handle batches of multiple protein on a
            # single GPU which yields better results
            if self.accum_iter != None and ((batch_idx + 1) % self.accum_iter == 0) or (batch_idx + 1 == len(self.data_loader)):
                    self.optimizer.step()
                    self.optimizer.zero_grad()
                    if self.optimizer_loss:
                        self.optimizer_loss.step()
                        self.optimizer_loss.zero_grad()

            # basic weights update if not using gradient
            # accumulation
            else:
                self.optimizer.step()
                self.optimizer.zero_grad()
