...
def residue_name_plot(resn_labels, fig, i, x_bounds):
    ax = fig.add_subplot(6*x_bounds+1, 1, 1+6*i+1)
    ...
    ax = fig.add_subplot(6*x_bounds+1, 1, 6+6*i+1)
...
def secondary_structure_plot(resn_labels, ss_elements, fig, i, x_bounds):
    ax = fig.add_subplot(6*x_bounds+1, 1, 4+6*i+1)
...
def rsa_plot(rsa_labels, fig, i, x_bounds):
    ax = fig.add_subplot(6*x_bounds+1, 1, 2+6*i+1)
...
def hp_plot(hsa_labels, fig, i, x_bounds):
    ax = fig.add_subplot(6*x_bounds+1, 1, 3+6*i+1)
    ...
    plt.fill_between(x_smooth, y_smooth, 0.25,
                     where=(0.25 > y_smooth), 
                     alpha=1, color='darkviolet', interpolate=True)
    ax.set_ylim([-0.05, 1])
...
def disorder_plot(dis_labels, fig, i, x_bounds):
    ax = fig.add_subplot(6*x_bounds+1, 1, 5+6*i+1)
...
def plot_features(name, resn_labels, ss_elements, rsa_labels, hp_labels, lhp_labels, dis_labels, glob, max_length=75):
    px = 1/plt.rcParams['figure.dpi']

    # Normalize HP_loc and LHP_loc
    if glob[2] != 0:
        hp_labels = hp_labels/max(hp_labels)
        lhp_labels = lhp_labels/max(lhp_labels)
    ...
    for i in range(x_bounds):
        ...
        _hp_labels = hp_labels[max_length*i:i*max_length+max_length]
        ...
        if glob[2] != 0:
            hp_plot(_hp_labels, fig, i, x_bounds)
    ...
    if glob[2]!=0:
        fig.text(215/width, 1 - 40/height, f'Id: {os.path.basename(name)}      TASA: {glob[0]:.2f}      THSA: {glob[1]:.2f}        RHSA: {glob[1]/glob[0]:.3f}        LHP: {glob[2]:.2f}        Species: {glob[3]}      Expression: {glob[4]}',fontsize = 15, fontfamily='serif')
    else:
        fig.text(215/width, 1 - 40/height, f'Id: {os.path.basename(name)}      TASA: {glob[0]:.2f}      THSA: {glob[1]:.2f}        RHSA: {glob[1]/glob[0]:.3f}        LHP: --        Species: {glob[3]}      Expression: {glob[4]}',fontsize = 15, fontfamily='serif')
    ...
    plt.savefig("plot_results/{}.png".format(name))
