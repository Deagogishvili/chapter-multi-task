...
class SecondaryFeatures(BasePredict):
    ...
    def postprocessing(self, x: torch.tensor, mask: torch.tensor):
        ...
        for i in range(x[0].shape[0]):
            ...
            x[9][i] = F.softmax(x[9][i], dim = 1)
            x[10][i] = F.softmax(x[10][i], dim = 1)
            ...
            for idx_m in range(math.ceil(len(fasta)/batch_size)):
                ...
                for i in range(len(identifier)):
                    ...
                    for j in range(len(identifier[i])):
                        ...
                        species, expression = SPECIES[np.argmax(np.sum(prediction[i][11][j,:list_mask[i][j]], axis=0))], EXP[np.argmax(np.sum(prediction[i][12][j,:list_mask[i][j]], axis=0))] #, torch.softmax(torch.sum(torch.from_numpy(prediction[i][13][j,:list_mask[i][j]]), axis=0), dim=1).detach().numpy()[1]
